 ...
 */
object Str {
    const val appName = "567 Agent"

    // Welcome / marketing
    const val welcomeTitle = "567 Agent\n随身智能助手"
    const val welcomeSubtitle = "连接您的 567 Agent 桌面端，随时随地跑代码、查项目、继续工作；亦可直接接入 567 API 畅享满血大模型。"
    const val featureDesktop = "对接 Desktop"
    const val featureDesktopDesc = "通过专属私人中继，实时操作电脑工作区"
    const val featureCloud = "567 API 对话"
    const val featureCloudDesc = "直连 api.567.wiki，支持各种旗舰大模型"
    const val featureStatus = "额度与状态"
    const val featureStatusDesc = "实时查询 567 API 账户可用余额与模型"
    const val featureSecure = "端到端加密"
    const val featureSecureDesc = "专属中继与安全密钥，保护隐私数据"
    const val getStarted = "登录 567 API"
    const val scanPairing = "扫码连接电脑 567 Agent"
    const val skipForNow = "稍后跳过"
    const val notLoggedIn = "未登录 567 API"
    const val notAvailable = "暂无"
    const val connectingDesktop = "正在连接 Desktop…"

    const val loginTitle = "登录 567 API"
    const val loginSubtitle = "使用 567 API (api.567.wiki) 账号密码或访问令牌登录"
    const val email = "访问令牌 (Token / PAT)"
    ...
    const val useAccountLogin = "使用 567 账号密码登录"
    const val useEmailLogin = "使用访问令牌 (Token) 登录"
    const val account = "567 API 用户名"
    ...
    const val versionNumber = "版本 1.0.4"
