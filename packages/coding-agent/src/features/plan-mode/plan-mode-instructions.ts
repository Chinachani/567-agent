...
export function renderPlanModeInstructions(options: { readonly canSubmitPlan: boolean }): string {
	return [
		...
		"   - for something built from scratch, the application itself: platform and tech stack, feature set and depth, look and feel, content and assets. There is no code to settle these, so they are the user's call.",
		...
		"   Walk each decision in dependency order: start with the ones that shape everything else, then use the answers to decide what is still worth asking. There is no fixed number of rounds. What ends the questioning is the list, not a count: keep going while a decision that would materially change the plan is still open, and stop the moment none is. A one-line request for a whole application normally takes several exchanges; a precise change to existing code often takes none.",
