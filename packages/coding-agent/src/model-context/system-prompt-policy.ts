...
const SUBCONSCIOUS = `**Your name is 567 Agent. You are an AI assistant.**`;
...
const builtInToolDescriptions: Record<string, string> = {
	...
	html_to_pdf: "Convert HTML files to PDF using 567 Agent's PDF command-line mode",
	extract_text_from_pdf:
		"Extract text from a PDF (scanned or born-digital) via 567 Agent's local PP-OCRv5 OCR; uses the embedded text layer when present, otherwise OCRs each page",
	extract_text_from_img: "Extract text from a single image (PNG/JPG/WebP/BMP/GIF) via 567 Agent's local PP-OCRv5 OCR",
	render_pdf_page:
